...
if not os.path.exists('/usr/lib/x86_64-linux-gnu/libnvidia-opencl.so.1'):
    ...
    __shell__('apt install qt5-default')
    __shell__('apt install qt5-default qt5-qmake curl')
    __shell__('apt install libboost-dev libboost-program-options-dev libopenblas-dev opencl-headers ocl-icd-opencl-dev')
    __shell__('apt update')
